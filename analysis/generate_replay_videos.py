...
def generate_latex_color_definitions():
    ...
    # Add the custom Anthropic orange colors
    ...
    if "anthropicDarkOrange" not in unique_defs:
        # Define the dark orange too, although we won't use it for 'W' based on current request
...
def render_latex_frame(step_data, agent_message_colors_map, coord_to_agent_id_map,
                      ...
                      game_steps=None, # <<<--- 添加game_steps参数
    ...
    # --- LaTeX Color Definitions ---
    # Ensure generate_latex_color_definitions() includes:
    # defs.append(f"\\definecolor{{anthropicOrange}}{{rgb}}{{0.93, 0.43, 0.2}}")
    # defs.append(f"\\definecolor{{anthropicDarkOrange}}{{rgb}}{{0.78, 0.31, 0.09}}") # Defined but unused here
    ...
    def render_single_grid_latex(grid, local_agent_message_colors_map, local_coord_to_agent_id_map):
        ...
        try:
            ...
            # Ensure grid is treated consistently, handle potential None values better
            ...
            for i in range(height):
                ...
                for j in range(width):
                    ...
                    try:
                        cell = grid_np[i, j]
                        # Treat empty strings like None, default to '.'
                        original_cell_str = str(cell).strip() if cell is not None and str(cell).strip() != '' else '.'
                    except IndexError:
                        original_cell_str = '?'; cell = '?' # Should not happen with pre-conversion

                    # --- 准备变量 ---
                    latex_cell_cmd = "\\termcelldef" # 默认 LaTeX 命令
                    latex_params = ["{.}"]           # 默认参数
                    ...
                    if agent_id_at_coord and agent_id_at_coord in local_agent_message_colors_map:
                        ...
                        display_content = agent_num_str # Agent 显示其编号
                        # Agent 编号通常是安全的，但以防万一还是转义
                        safe_display_content = str(display_content).replace('_','\\_').replace('{','\\{').replace('}','\\}')
                        padded_safe_content = "{:<2}".format(safe_display_content) # 填充 Agent 编号

                        message_color = local_agent_message_colors_map[agent_id_at_coord]
                        if original_cell_str == 'a': # 如果原始字符是 'a' (Agent 标记) - 仅着色前景
                        ...
                        else: # Agent 在其他类型的单元格上 - 同时着色背景和前景
                            grid_back_color, grid_text_color = AGENT_GRID_COLORS_MAP.get(message_color, DEFAULT_GRID_AGENT_COLOR)
                            bg_color_name, _ = COLORAMA_TO_LATEX.get(grid_back_color, ("termBgWhite", "")) # 默认 Agent 背景
                            fg_color_name, _ = COLORAMA_TO_LATEX.get(grid_text_color, ("termBlack", "")) # 默认 Agent 文本
                    ...
                    # --- B: 处理非 Agent 单元格 ---
                        ...
                        if original_cell_str == 'W':
                            latex_cell_cmd = "\\termcell"
                            bg_color_name = "anthropicOrange"  # 使用自定义橙色
                            fg_color_name = "termBlack"        # 使用黑色文本
                            padded_safe_content = "{:<2}".format('W') # 确保显示 'W' 并填充
                            latex_params = [f"{{{bg_color_name}}}", f"{{{fg_color_name}}}", f"{{{padded_safe_content}}}"]
                            ...
                            latex_cell_cmd="\\termcell"
                            latex_params=[f"{{{bg_name}}}", f"{{{fg_name}}}", f"{{{padded_safe_content}}}"] # 使用原始字符的填充版本
                            ...
                            fg_name,_=COLORAMA_TO_LATEX.get(fore_c,("termWhite",""))
                            latex_cell_cmd="\\termcell"
                            padded_safe_content = "{:<2}".format('A') # 确保显示 'A' 并填充
                            latex_params=[f"{{{bg_name}}}", f"{{{fg_name}}}", f"{{{padded_safe_content}}}"]
                        elif original_cell_str == '.':
                            latex_cell_cmd = "\\termcelldef" # 使用默认单元格命令
                            padded_safe_content = "{:<2}".format('.') # 确保显示 '.' 并填充
                            latex_params = [f"{{{padded_safe_content}}}"]
                        # *** 修改点：处理 F, N 和其他所有未特殊处理的字符 ***
                        else:
                            # 使用默认的 LaTeX 单元格命令 (\termcelldef)
                            # 使用之前计算好的、基于 original_cell_str 的 padded_safe_content
                            # 这将正确显示 'F', 'N', 或任何其他字符，并应用默认颜色和填充
                            latex_cell_cmd = "\\termcelldef"
                            latex_params = [f"{{{padded_safe_content}}}"]

                    # --- 构建最终的 LaTeX 单元格字符串 ---
                    # latex_cell_cmd 和 latex_params 已在 Agent 或 Non-Agent 块中设置好
    ...
    # --- Rest of your script ---
    # 使用 [H] 选项固定位置，并添加居中对齐
    latex_string += "\\begin{figure}[p] % Allow figure to take a full page if needed\n"
    ...
    # 使用minipage创建固定高度的消息区域，并设置固定字体大小
    latex_string += "\\vspace{1em}\n"
    latex_string += "\\noindent\\begin{minipage}[t][8cm][t]{\\textwidth} % 固定高度为8cm的消息区域\n"
    latex_string += "\\fontsize{8pt}{9pt}\\selectfont % 固定字体大小和行高\n"
    if message_block:
    ...
    try:
        if game_steps:
            for i, game_step in enumerate(game_steps[:current_step_index + 1]):
                if isinstance(game_step, dict) and 'score' in game_step:
                    try:
                        # 确保分数正确转换为数字
                        score_str = str(game_step['score']).strip()
                        # 去除任何非数字字符，保留小数点
        ...
        if not all_scores:
            all_scores = [(0, 0.0)]
        latex_string += "% Error processing scores: " + str(e) + "\n"
    ...
    try:
        if all_scores:
            ...
            if min_score == max_score:
                ...
                min_score = max(0, min_score - 0.1 * range_size)
                max_score = max_score + 0.1 * range_size
            
            latex_string += f"    ymin={min_score}, ymax={max_score},\n"
        else:
    ...
    # 添加坐标点
    for frame, score in all_scores:
        latex_string += f"    ({frame},{score})\n"
    ...
    try:
        if all_scores:
            current_score = all_scores[-1][1]
            latex_string += f"\\node[circle, fill=red, inner sep=2pt] at (axis cs:{current_step_index},{current_score}) {{}};\n"
    except Exception:
        # 如果出错，不添加标记
        pass
    ...
    for i, step in enumerate(game_steps):
        ...
        try:
            latex_content = render_latex_frame(
                ...
                game_steps=game_steps,  # 添加game_steps参数
